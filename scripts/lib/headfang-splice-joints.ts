...
import {HEADFANG_SPLICE as joint} from '../../src/model/headfang-splices';
import type {Catalog,Part} from '../../src/model/types';

/** Endpoint-only machining preserves the existing central crossing and carved underside. */
export function buildHeadfangSpliceJoints(catalog:Catalog,assignments:Record<string,string>,meshes:Record<string,unknown>,components:(g:BufferGeometry)=>number){
 ...
 const cut=(p:Part,k:Brush)=>{const old=world(p),next=evaluator.evaluate(old,k,SUBTRACTION);k.geometry.dispose();
  if(!next.geometry.getAttribute('position').count||components(next.geometry)!==1)throw new Error(`Headfang splice splits ${p.id}`);
  old.geometry.dispose();next.updateMatrixWorld();live.set(p.id,next);
 };
 for(const span of catalog.parts.filter(p=>p.shape==='splicedFangSpan')){
  ...
  // The first two dependencies are the two column endpoints. Later dependencies
  // are transverse intercolumn timbers, which must never receive endpoint cuts.
  for(const id of span.requires.slice(0,2)){
   const node=by.get(id)!;if(!/^第[1-5]层柱头枋节点$/.test(node.kind))throw new Error(`Unexpected headfang endpoint ${id}`);
   const side=Math.sign(new Vector3(...node.position).sub(new Vector3(...span.position)).dot(t)),centre=side*(span.size[0]/2-joint.overlap/2),length=joint.overlap+2*joint.clearance;
   cut(node,box(centre,5+joint.seat,0,length,10,joint.tongue+2*joint.clearance));
   for(const sign of [-1,1])cut(span,box(centre,0,sign*(joint.tongue/2+5),length,10,10));
  }
 }
